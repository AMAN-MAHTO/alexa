...
# to automate some keybords comment
...
# engine of jarvis to speak
engine = pyttsx3.init("sapi5")
rate = engine.getProperty('rate')
engine.setProperty('rate', rate - 50)
voices = engine.getProperty("voices")
engine.setProperty("voice", voices[0].id)  # selecting language of jarvis 0,1
...
try:
    ...
    dbfile = sqlite3.connect(f"{current_path}\\QuestionAnsw.db")
    curs = dbfile.cursor()
    ...
    # exception if path table not find
        ...
        cours = dbfile.cursor()
        cours.execute(
            '''CREATE TABLE path(name STRING PRIMARY KEY,path STRING);''')
        dbfile.commit()
...
def askqus(qus):
    ...
    try:
        ...
        try:
            ...
            ans = 'unknown'
            curs.execute(
                "INSERT INTO questions(qus,ans) VALUES(?,?);", (qus, ans))
            dbfile.commit()
            print(e)
                    ...
                    try:
                        ...
                        for path_char in copy_path:
                            ...
                            if path_char == '\\':
                                gen_path += path_char
                        curs.execute(
                            "INSERT INTO path(name,path) VALUES(?,?);", (quary, gen_path))
                        dbfile.commit()
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if quary_list[0] == 'jarvis':
            quary = quary[6::]       # removing jarvis word from string
            quary_list.pop(0)
            if quary == '':
            ...
            elif quary_list[0] == 'inspect':
                hotkey('ctrl', 'shift', 'i')
            elif quary_list[0] == "join":
                if "chemistry" in quary:
                    speak("joining chemistry class")
                    webbrowser.open(
                        "https://meetingsapac30.webex.com/meet/pr1566859408")
                ...
                speak(
                    "I am jarvis.I am your assistant.I accept all your visitors as my reason and follow them is my task"
                    "I take tke the voice command then recognize it and give you output"
                    "I have some spacial feature"
                    " like i can open youtube, google , search on wikipidea,search on pc, play musics, even i copy you as nimickri articts just by giving the command 'copy me jarvis' ")
            elif "hello" in quary:
            ...
            elif quary_list[0] == 'thankyou':
                speak("its my owner sir")
            elif quary_list[0] == 'bye' or quary_list[0] == 'buy':
                speak("i think, i am use full for you Thank you bye! ")
                exit()
            # simple commands
            elif quary_list[0] == "minimise":
                hotkey("win", "down")
            elif quary_list[0] == "minimise" and quary_list[1] == 'all':
                hotkey("win", "d")
